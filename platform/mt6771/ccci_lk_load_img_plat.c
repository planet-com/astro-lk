...
#define CCB_DATA_BUF_DEFAULT_GEAR 1
...
static unsigned int get_ccb_gear_val(void)
{
	...
	if (md1_ccb_cap_gear != 0) {
		...
		ALWAYS_LOG("[%s] MTK_DYNAMIC_CCB_BUFFER_GEAR_ID: %d\n",
				__func__, MTK_DYNAMIC_CCB_BUFFER_GEAR_ID);
#else
		md1_ccb_cap_gear = CCB_DATA_BUF_DEFAULT_GEAR;
		ALWAYS_LOG("[%s] CCB_DATA_BUF_DEFAULT_GEAR: %d\n",
				__func__, CCB_DATA_BUF_DEFAULT_GEAR);
#endif
