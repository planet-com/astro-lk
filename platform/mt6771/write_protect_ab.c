...
void set_write_protect(void)
{
	...
	if (g_boot_mode == NORMAL_BOOT) {
		...
		if (s > 0 && e > 0) {
			...
			pal_log_err("[%s]: Unknown error! \n", __func__);
		}
		memset(wp_start, 0, sizeof(char) * WRITE_PROTECT_PARTITION_NAME_SZ);
		memset(wp_end, 0, sizeof(char) * WRITE_PROTECT_PARTITION_NAME_SZ);
		s= snprintf(wp_start, WRITE_PROTECT_PARTITION_NAME_SZ, "md1img%s", ab_suffix);
#ifdef MTK_SECURITY_SW_SUPPORT
		t = snprintf(tmp_name, WRITE_PROTECT_PARTITION_NAME_SZ, "system%s", ab_suffix);
		if (t > 0) {
			if (TRUE == seclib_sec_boot_enabled(TRUE) &&
				PART_OK == partition_exists(tmp_name)) {
				e = snprintf(wp_end, WRITE_PROTECT_PARTITION_NAME_SZ, "system%s", ab_suffix);
				/* WP for super partition is disabled when AB system is enabled. */
			} else
				e = snprintf(wp_end, WRITE_PROTECT_PARTITION_NAME_SZ, "tee%s", ab_suffix);
		}
		else {
			e = snprintf(wp_end, WRITE_PROTECT_PARTITION_NAME_SZ, "tee%s", ab_suffix);
		}
