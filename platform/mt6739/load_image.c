...
int mboot_android_load_bootimg_hdr(char *part_name, uint32_t addr)
{
	...
	if (g_is_64bit_kernel == 1) {
		n = snprintf(cmdline, UART_EARLY_CONSOLE_LEN, " earlycon=uartmtk,mmio32,0x%x,", AP_UART0_BASE);
	} else {
		n = snprintf(cmdline, UART_EARLY_CONSOLE_LEN, " earlyprintk=mt6739,0x%x ", AP_UART0_BASE);
	}
	if (n > 0)
