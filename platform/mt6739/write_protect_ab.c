...
void set_write_protect(void)
{
	...
	if (g_boot_mode == NORMAL_BOOT) {
		...
		err = (snprintf(wp_start, WRITE_PROTECT_PARTITION_NAME_SZ, "sec1") > 0) ? 0 : INVALID_BUFFER_SIZE;
		err |= (snprintf(wp_end, WRITE_PROTECT_PARTITION_NAME_SZ, "logo") > 0) ? 0 : INVALID_BUFFER_SIZE;
		if (0 == err) {
			...
			pal_log_err("[%s] Fail to set write protect partition\n", __func__);
		}

		memset(wp_start, 0, sizeof(char) * WRITE_PROTECT_PARTITION_NAME_SZ);
		memset(wp_end, 0, sizeof(char) * WRITE_PROTECT_PARTITION_NAME_SZ);

		err = (snprintf(wp_start, WRITE_PROTECT_PARTITION_NAME_SZ, "md1img%s", ab_suffix) > 0) ? 0 : INVALID_BUFFER_SIZE;
#ifdef MTK_SECURITY_SW_SUPPORT
		err |= (snprintf(tmp_name, WRITE_PROTECT_PARTITION_NAME_SZ, "system%s", ab_suffix) > 0) ? 0 : INVALID_BUFFER_SIZE;
		if (0 == err &&
			TRUE == seclib_sec_boot_enabled(TRUE) &&
			PART_OK == partition_exists(tmp_name)) {
			err |= (snprintf(wp_end, WRITE_PROTECT_PARTITION_NAME_SZ, "system%s", ab_suffix) > 0) ? 0 : INVALID_BUFFER_SIZE;
			/* WP for super partition is disabled when AB system is enabled. */
		} else
			err |= (snprintf(wp_end, WRITE_PROTECT_PARTITION_NAME_SZ, "tee%s", ab_suffix) > 0) ? 0 : INVALID_BUFFER_SIZE;
#else
		err |= (snprintf(wp_end, WRITE_PROTECT_PARTITION_NAME_SZ, "tee%s", ab_suffix) > 0) ? 0 : INVALID_BUFFER_SIZE;
#endif
		if (0 == err) {
