...
__WEAK void ufs_get_platform_data(void)
{
	...
	ufs_platform.highspeed_gear = UFS_HS_G3;
}
...
static void ufshcd_init_custom_info(struct ufs_hba *hba)
{
	...
	UFS_DBG_LOGI("[UFS] Custom force provison=%d, TW size=%dGB, TW no red=%d, HPB size=%dGB\n",
...
static int ufs_aio_pre_pwr_change(struct ufs_hba *hba, struct ufs_pa_layer_attr *desired, struct ufs_pa_layer_attr *final)
{
	...
	if (final->pwr_rx == SLOW_MODE || final->pwr_rx == SLOWAUTO_MODE) {
		...
		final->gear_rx = min_t(u32, ufs_platform.highspeed_gear, desired->gear_rx);
		final->gear_tx = min_t(u32, ufs_platform.highspeed_gear, desired->gear_tx);
		if (ufs_platform.highspeed_gear == UFS_HS_G4) {
...
#ifdef MTK_UFS_DRV_DA
		if ((hba->dev_info.hpb_support) && (i == 2) && (p[CONF_DESC_UNIT_B_LU_ENABLE] == 1) && (hba->custom_info.hpb_size_gb != 0)) {
...
int ufs_aio_configure_new_ufs(struct ufs_hba *hba)
{
	...
	d_num_alloc_units_boot_lu = ufs_aio_get_lu_cfg_size(hba, &info, UFS_LU_0);
	d_num_alloc_units_user_lu = ufs_aio_get_lu_cfg_size(hba, &info, UFS_LU_2);
	d_num_alloc_units_lu3     = ufs_aio_get_lu_cfg_size(hba, &info, UFS_LU_3);
...
int ufs_aio_get_device_info(struct ufs_hba *hba)
{
	...
	if ((card_data->wmanufacturerid == UFS_VENDOR_SAMSUNG) || (card_data->wmanufacturerid == UFS_VENDOR_MICRON_MP)) {
		err = ufshcd_init_vendor_desc_sizes(hba); /* samsung/micron need use selector = 1 to read hpb support */
...
skip_selector:
	...
	UFS_DBG_LOGI("bbUD0BaseOffset = %d, bUDConfigPLength = %d\n", card_data->ud0_base_offset, card_data->ud_config_len);
