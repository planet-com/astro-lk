...
static struct mtk_spi_bus_config spi_default_config = {
	.spi_mode = 0,
	.tick_delay = 0,
};
...
static void mtk_spi_packet(int bus_num, u32 size)
{
	u32 reg_val, packet_len, packet_loop;
	struct mtk_spi_regs *regs = spi_bus[bus_num]->reg_addr;
...
static int mtk_spi_fifo_transfer(int bus_num, unsigned char *rx_buf,
		unsigned char *tx_buf, u32 size)
{
	u32 i, reg_val = 0, word_count, remaind;
	int ret;
	struct mtk_spi_regs *regs = spi_bus[bus_num]->reg_addr;
		...
		unsigned char *tx_buf, u32 size)
{
	...
	if (tx_buf) {
		...
		mt_reg_sync_writel((u32)tx_buf, &regs->spi_tx_src_reg);
	}

	if (rx_buf) {
		arch_clean_cache_range((addr_t)rx_buf, size);
		mt_reg_sync_writel((u32)rx_buf, &regs->spi_rx_dst_reg);
	}
...
int spi_sync(int bus_num, struct spi_transfer *transfer)
{
	...
	if(bus_num < 0) {
		dprintf(CRITICAL, "spi: bus_num is valid.\n");
		return -1;
	}
				...
				const char *name, unsigned int *out_value)
{
	...
	int len = 0;
	void *lk_drv_fdt = (void *)get_lk_overlayed_dtb();
			...
			struct mtk_spi_bus_config *config)
{
	...
	if (node <= 0) {
		dprintf(SPEW, "spi slave dts node is not set, just skip.\n");
		return -1;
	}
	...
	if (!bus) {
		dprintf(CRITICAL, "faild to malloc mtk_spi_bus\n");
		return -1;
	...
	spi_bus[spi_bus_num]->reg_addr = (struct mtk_spi_regs *)temp_data[1];
	spi_bus[spi_bus_num]->pad_select = pad_select;
	spi_bus[spi_bus_num]->spi_bus_fdt_offset = parent_node;
	dprintf(SPEW, "reg addr : %p, pad_select : %d\n",
