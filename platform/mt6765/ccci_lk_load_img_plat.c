...
static int cal_share_mem_layout(int load_flag)
{
	...
	if (md1_ccb_size != 0)
		ccb_data_buf = ccci_request_named_mem("ap_md_c_smem", md1_ccb_size);
...
int plat_apply_platform_setting(int load_md_flag)
{
	...
	if (load_md_flag & (1<<MD_SYS1)) {
		...
		/* remapping CCB to bank4 slot 3,4*/
		md_bank4_remapping_by_slot(MD_SYS1, ccb_info.ccb_data_buffer_addr, 3);
		md_bank4_remapping_by_slot(MD_SYS1, ccb_info.ccb_data_buffer_addr + 0x2000000, 4);
		md_bank4_remapping_by_slot(MD_SYS1, ccb_info.ccb_data_buffer_addr + 0x4000000, 5);
