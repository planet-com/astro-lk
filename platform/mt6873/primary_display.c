...
{
	return __primary_display_trigger(blocking, false);
}
...
#ifndef MACH_FPGA_NO_DISPLAY
	display_height = primary_display_get_height();
#else	
	display_height = LCM_MAX_HEIGHT;
#endif
	
	return ALIGN_TO(DISP_GetScreenWidth(), MTK_FB_ALIGNMENT) * ALIGN_TO(display_height, MTK_FB_ALIGNMENT) * ((DISP_GetScreenBpp() + 7) >> 3) * DISP_GetPages();
}
...
/*for MRDUMP buffer*/
UINT32 DISP_GetMBRamSize(void)
{
	static UINT32 mramSize = 0;
	UINT32 width = 0;
	UINT32 height = 0;
	UINT32 bpp = 0;
	if (0 == mramSize){
		width = ALIGN_TO(DISP_GetScreenWidth(), MTK_FB_ALIGNMENT);
		if (mrdump_check_enable() <= MRDUMP_ALWAYS_ENABLE && current_is_abnormal_boot())
			height = ALIGN_TO(DISP_GetScreenHeight(), MTK_FB_ALIGNMENT);
		else
			height = ALIGN_TO(DISP_GetScreenHeight() / 8, MTK_FB_ALIGNMENT);
		bpp = (DISP_GetScreenBpp() + 7) >> 3;
		mramSize = width * height * bpp;
		DISPDBG("^^ DISP_GetMBRamSize: %d, %d, %d\n", width, height, bpp);
	}
	DISPDBG("^^ DISP_GetMBRamSize: %u bytes\n", mramSize);
	return mramSize;
}
