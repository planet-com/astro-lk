...
void mcupm_do_regdump(uint32_t *out, uint32_t *out_end)
{
	...
	dfd_status = DRV_Reg32(DFD_STATUS);

	if (dfd_status == 0) {
		DRV_WriteReg32(buf, DRV_Reg32(MCUPM_SW_RSTN +
					CPUEB_CFGREG_DBG_MON_PC));
		DRV_WriteReg32(buf+1, DRV_Reg32(MCUPM_SW_RSTN +
					CPUEB_CFGREG_DBG_MON_LR));
		DRV_WriteReg32(buf+2, DRV_Reg32(MCUPM_SW_RSTN +
					CPUEB_CFGREG_DBG_MON_SP));
	}
	else {
...
#endif
	memcpy(buf, (DRV_Reg32(MCUPM_LAST_TBUF) + MCUPM_SRAM_BASE), sizeof(tbuf));
}
