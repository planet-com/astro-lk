...
UINT32 mt_disp_get_vram_size(void)
{
	if (mrdump_check_enable() <= MRDUMP_ALWAYS_ENABLE)
	        return (DISP_GetVRamSize() + DISP_GetMBRamSize());
        else
...
void mt_disp_init(void *lcdbase)
{
	...
	if (mrdump_check_enable() <= MRDUMP_ALWAYS_ENABLE && current_is_abnormal_boot())
	{
		memset((void*)(lcdbase + DISP_GetVRamSize()), 0x0, mt_disp_get_mram_size());
	}
	else 
	{
...
void __mt_disp_update(UINT32 x, UINT32 y, UINT32 width, UINT32 height, bool logo)
{
...
void mt_disp_update_no_logo(UINT32 x, UINT32 y, UINT32 width, UINT32 height)
{
