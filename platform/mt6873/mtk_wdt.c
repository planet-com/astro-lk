...
static void mtk_wdt_reset(char bypass_power_key)
{
	...
	/* Watchdog Rest */
	unsigned int wdt_mode_val, nonrst2;
	...
	wdt_mode_val = wdt_mode_val | MTK_WDT_MODE_KEY | MTK_WDT_MODE_EXTEN |
			 MTK_WDT_MODE_IRQ_LEVEL_EN | MTK_WDT_MODE_EXTRA_CNT;
				    ...
				    BOOL wdt_en)
{
	...
	tmp = DRV_Reg32(MTK_WDT_MODE);
	tmp |= MTK_WDT_MODE_KEY | MTK_WDT_MODE_IRQ_LEVEL_EN | MTK_WDT_MODE_EXTRA_CNT;
