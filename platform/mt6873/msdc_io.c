...
{
#if defined(MTK_MSDC_BRINGUP_DEBUG)
	msdc_pr_info("MSDC0 HCLK_MUX[0x1000_0070][25:24] = %d, pdn[31] = %d, "
		"CLK_MUX[0x1000_0080][2:0] = %d, pdn[7] = %d, "
		"CLK_CG[0x11f1_0000] bit1 = %d, bit4 = %d, bit8 = %d\n"
		"CLK_CG[0x11f1_0000] bit9 = %d, bit14 = %d\n",
		/* mux at bits 25~24 */
		...
		/* cg at bit 1, 4, 8 represent sclk & pclk & AXI */
		/* cg at bit 9,14 represent hclk & AHB2AXI*/
		(MSDC_READ32(0x11f10000) >> 1) & 1,
		(MSDC_READ32(0x11f10000) >> 4) & 1,
		(MSDC_READ32(0x11f10000) >> 8) & 1,
		(MSDC_READ32(0x11f10000) >> 9) & 1,
		(MSDC_READ32(0x11f10000) >> 14) & 1);
#endif
}
#endif
...
void msdc_get_driving_by_id(u32 id, struct msdc_cust *msdc_cap, bool sd_18)
{
	if (id == 0) {
		...
		MSDC_GET_FIELD(MSDC0_GPIO_DRV0_ADDR, MSDC0_DRV0_CLK_MASK,
			msdc_cap->clk_drv);
		MSDC_GET_FIELD(MSDC0_GPIO_DRV0_ADDR, MSDC0_DRV0_DAT_MASK_0,
			msdc_cap->dat_drv);
	}
}
#endif
