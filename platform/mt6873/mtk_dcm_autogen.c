...
#define INFRACFG_AO_MTS_BUS_DCM_REG0_MASK ((0x1U << 31))
#define INFRACFG_AO_MTS_BUS_DCM_REG0_ON ((0x1U << 31))
...
void dcm_mp_cpusys_top_cpu_pll_div_3_dcm(int on)
{
	if (on) {
		...
		reg_write(CPU_PLLDIV_CFG3,
			(reg_read(CPU_PLLDIV_CFG3) &
			~MP_CPUSYS_TOP_CPU_PLL_DIV_3_DCM_REG0_MASK) |
...
void dcm_mp_cpusys_top_cpu_pll_div_4_dcm(int on)
{
	if (on) {
		...
		reg_write(CPU_PLLDIV_CFG4,
			(reg_read(CPU_PLLDIV_CFG4) &
			~MP_CPUSYS_TOP_CPU_PLL_DIV_4_DCM_REG0_MASK) |
