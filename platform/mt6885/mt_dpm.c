...
int load_dpm(void)
{
	...
	strncpy (part_name,"dpm",sizeof("dpm"));
...
static int verify_load_DPM_image(char *part_name, void *addr)
{
	...
	dprintf(INFO, "[DPM] %s: pm_ptr=%p pm_size=%p dm_ptr=%p dm_size=%p\n", __func__, pm_ptr, pm_size, dm_ptr, dm_size);
	if (pm_size > 0x6000) { //pm > 24K
		...
		//DRV_WriteReg32(DPM_CFG_CH1, DRV_Reg32(DPM_CFG_CH1)|0x0);
	}
	else if (pm_size > 0x2000) { //pm > 8K
		DRV_WriteReg32(DPM_CFG_CH0, DRV_Reg32(DPM_CFG_CH0)|0x10000000);
		DRV_WriteReg32(DPM_CFG_CH1, DRV_Reg32(DPM_CFG_CH1)|0x10000000);
	}
	else { //pm < 8K
		DRV_WriteReg32(DPM_CFG_CH0, DRV_Reg32(DPM_CFG_CH0)|0x20000000);
		DRV_WriteReg32(DPM_CFG_CH1, DRV_Reg32(DPM_CFG_CH1)|0x20000000);
	}
	...
	************************/
	...
	memcpy((void *) DPM_PM2_SRAM_BASE, pm_ptr, pm_size);
	memcpy((void *) DPM_DM1_SRAM_BASE, dm_ptr, dm_size);
	memcpy((void *) DPM_DM2_SRAM_BASE, dm_ptr, dm_size);

	// step 7: write bootargs
	DRV_WriteReg32(DPM_BARGS_CH0_REG0, 0x00000000);
	DRV_WriteReg32(DPM_BARGS_CH0_REG1, 0x00000000);
	DRV_WriteReg32(DPM_BARGS_CH1_REG0, 0x80000000);
	DRV_WriteReg32(DPM_BARGS_CH1_REG1, 0x80000000);

	dsb();

	//RST of CH0 and CH1 
	DRV_WriteReg32(DPM_CFG_CH0, DRV_Reg32(DPM_CFG_CH0)|0x1);
	DRV_WriteReg32(DPM_CFG_CH1, DRV_Reg32(DPM_CFG_CH1)|0x1);
