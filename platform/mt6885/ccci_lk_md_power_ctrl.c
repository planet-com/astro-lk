...
static void md_power_on(void)
{
	...
	md_power_value = mt_secure_call_ret4(MTK_SIP_BL_CCCI_CONTROL_AARCH32,
		MD_POWER_CONFIG,MD_LK_BOOT_UP, 0, 0, &r1, &r2, 0);
	dprintf(CRITICAL, "md_power_value=%d, HW code =0x%X, SW_version=0x%X\n",
		md_power_value, r1, r2);
