...
void dcm_dump_regs(void)
{
	...
	/* big core stall */
	REG_DUMP(CPU4_STALL_DCM_CONF);
	REG_DUMP(CPU5_STALL_DCM_CONF);
	REG_DUMP(CPU6_STALL_DCM_CONF);
	REG_DUMP(CPU7_STALL_DCM_CONF);
...
static unsigned int disabled_by_dts(char *prop)
{
	...
	nodeoffset = fdt_node_offset_by_compatible(lk_drv_fdt, -1, "mediatek,mt6885-dcm");
	if (nodeoffset < 0) {
...
int mt_dcm_init(void)
{
	...
	if (mcu_disabled && infra_disabled) {
		...
		dcm_set_state(ARMCORE_DCM_TYPE | MCUSYS_DCM_TYPE | STALL_DCM_TYPE | BIG_CORE_DCM_TYPE | INFRA_DCM_TYPE, DCM_OFF);
	} else if (mcu_disabled) {
		dcm_warn("[DCM] MCU DCM is disabled by DTS\n");
		dcm_set_state(ARMCORE_DCM_TYPE | MCUSYS_DCM_TYPE | STALL_DCM_TYPE | BIG_CORE_DCM_TYPE,  DCM_OFF);
