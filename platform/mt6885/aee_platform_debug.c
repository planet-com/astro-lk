...
static unsigned int save_mcupm_coredump(u64 offset, int *len, CALLBACK dev_write)
{
	unsigned int *buf = NULL;
	unsigned int datasize = 0;
	int retry = MCUPM_COREDUP_RETRY;

	if (!(*(unsigned int *)MCUPM_BACKUP)) {
		return 0;
	}

	do {
		buf = *(unsigned int **)(MCUPM_DM_ADDR);
		*len = readl(MCUPM_DM_SZ);
		dprintf(INFO, "MCUPM_DM_ADDR 0x%x\n", MCUPM_DM_ADDR);
		dprintf(INFO, "MCUPM_DM_SZ = 0x%x\n", MCUPM_DM_SZ);
		dprintf(INFO, "len = 0x%x\n", *len);
		/* consider the condition that extra size more than current size, 0xba00 */
		if (*len > 0 && *len < 0xfa00) {
			dprintf(INFO, "capture DM len 0x%x\n", *len);
			datasize = dev_write( buf, *len);
			break;
		} else {
			dprintf(INFO, "DM len 0x%x, not capture DM\n", *len);
			udelay(100);
		}
	} while ( --retry);

	buf = *(unsigned int **)(MCUPM_RM_ADDR);
	*len = readl(MCUPM_RM_SZ);
	dprintf(INFO, "MCUPM_RM_ADDR 0x%x\n", MCUPM_RM_ADDR);
	dprintf(INFO, "len = 0x%x\n", *len);
	/* consider the condition that extra size more than current size, 0x80 */
	if (*len > 0 && *len < 0x400) {
		dprintf(INFO, "capture RM len 0x%x\n", *len);
		datasize += dev_write( buf, *len);
	} else {
		dprintf(INFO, "RM len 0x%x, not capture RM\n", *len);
	}
...
static unsigned int save_dpm_data(u64 offset, int *len, CALLBACK dev_write)
{
	...
	*len = (unsigned int)(DPM_CFG2_LEN);
	...
	buf = (unsigned int *)(DPM_DM2_SRAM_BASE);
	*len = (unsigned int)(DPM_DM_OFFSET);
	datasize += dev_write(buf, *len);

	buf = (unsigned int *)(DPM_DBG_LATCH_CH1);
	*len = (unsigned int)(DPM_DBG_LEN);
	datasize += dev_write(buf, *len);

	buf = (unsigned int *)(DPM_CFG1_CH1);
	*len = (unsigned int)(DPM_CFG1_LEN);
	datasize += dev_write(buf, *len);

	buf = (unsigned int *)(DPM_CFG2_CH1);
	*len = (unsigned int)(DPM_CFG2_LEN);
	datasize += dev_write(buf, *len);
...
/* GPU DFD */
#define dump_by_log 0
#define MFG_DUMP_BASE 0x13E00000
extern unsigned int mfg_set_power_on(void);
extern unsigned int mfg_set_power_off(void);
extern void show_gpu_dfd_debug_msg(void);
static unsigned int save_gpu_dfd_data(u64 offset, int *len, CALLBACK dev_write)
{
	struct aee_db_file_info *adfi = get_file_info();
	unsigned int filesize;
	char *buf = NULL;
	unsigned int i, length = 0, datasize = 0, ret;

	/* return when MFG not power on */
	if (!mfg_set_power_on())
		return 0;

	/* prepare buf */
	adfi += AEE_PLAT_GPU_DFD;
	filesize = adfi->filesize;
	buf = malloc(filesize);
	if (!buf) {
		dprintf(CRITICAL, "%s: malloc failed. (filesize = %u)\n", __func__, filesize);
		return 0;
	}
...
#if dump_by_log
	/* dump gpu dfd data */
	length = snprintf(buf, (filesize - 1), "[GPU_DFD]\n");
	for (i = MFG_DUMP_BASE; i <= MFG_DUMP_BASE + 0xFFFFC; i += 4) {
		ret = snprintf(buf + length, ((filesize - 1) - length),
			"0x%08x:0x%08x\n", i, DRV_Reg32(i));
		if (ret == -1)
			break;
		length += ret;
...
#else
	/* dump gpu dfd data
	 * 0x13e00000 ~ 0x13effffc
	 */
	 memcpy(buf, (unsigned int *)MFG_DUMP_BASE, 0xFFFFF);
	 length = 0xFFFFF;
#endif

	if (length > 0)
		datasize = dev_write((unsigned int*)buf, length);

	free(buf);

	mfg_set_power_off();

	dprintf(CRITICAL, "%s: dump dfd end \n", __func__);
...
#ifdef MTK_GPU_DFD_SUPPORT
	plat_gpu_dfd_get = save_gpu_dfd_data;
#endif
