	...
	switch (md_id) {
		case MD_SYS1:
			...
			lk_secure_ret = mt_secure_call_ret4(MTK_SIP_BL_CCCI_CONTROL_AARCH32,
				MD_POWER_CONFIG, MD_LK_BOOT_UP, 0, 0, &r1, &r2, 0);
			dprintf(CRITICAL, "lk_secure_ret=%d, HW code =0x%x, SW_version=0x%x\n",
				lk_secure_ret, r1, r2);
#else
...
#if 0
		ccci_write32(MDPERIMISC_BASE, 0x110C, 0x5500);
		// write 0x0 to boot slave jump address
		ccci_write32(MDPERIMISC_BASE, 0x1104, 0x0);
		// update boot slave jump address
		ccci_write32(MDPERIMISC_BASE, 0x1108, 0x1);
		dprintf(CRITICAL, "enter bypass md brom\n");
		return 0;
#endif
