...
/*---------------------------------------------------------------------------*/
S32 mt_set_gpio_driving(u32 pin, u32 drv)
{
	u32 mask;
	...
	GPIO_CLR_BITS(DRV_addr[pin].addr, mask << DRV_offset[pin].offset);
	GPIO_SET_BITS(DRV_addr[pin].addr, drv << DRV_offset[pin].offset);
	return RSUCCESS;
}
/*---------------------------------------------------------------------------*/
S32 mt_get_gpio_driving(u32 pin)
{
	u32 reg, mask;
	...
	mask = (1 << DRV_width[pin].width) - 1;
	return ((reg >> DRV_offset[pin].offset) & mask);
}
/*****************************************************************************/
/* Self test operation                                                           */
/*****************************************************************************/
