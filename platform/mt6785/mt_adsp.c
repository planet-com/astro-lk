...
static void set_adsp_mpu_region(void);
...
int load_adsp_image(char *part_name, char *img_name, void *addr)
{
	...
	dprintf(CRITICAL, "%s load %s addr=0x%x\n",__func__, img_name, (uint32_t)addr);
...
int load_adsp(void)
{
	...
	if (!dram_phys_addr) {
		...
		dprintf(INFO, "[ADSP] mblock_reserve success. phys view=0x%x, size=0x%x\n",
			(uint32_t)dram_phys_addr, g_total_dram_size);
...
static uint32_t parse_adsp_memory_from_dt(void)
{
	...
	/* system from dts */
	ret = get_adsp_sysram_info_from_dt();
	if (ret) {
		dprintf(CRITICAL, "get adsp sysrm info fail. ret=%d\n", ret);
	}
	adsp_dram_head = (uint32_t)image_table[ADSP_A_IMAGE_DRAM].base;
	adsp_dram_tail = (uint32_t)image_table[ADSP_A_IMAGE_DRAM].base + image_table[ADSP_A_IMAGE_DRAM].size;
	if (!adsp_dram_head || !adsp_dram_tail ) {
		dprintf(CRITICAL, "%s error: adsp dram size is NULL!\n", __func__);
		return 0;
	}

	return (uint32_t)image_table[ADSP_A_IMAGE_DRAM].size;
}
	...
	*/
	...
	for (mem_id = 0; mem_id < AUDIO_MEM_TOTAL_NUM; mem_id ++) {
		...
		if (data) {
			...
			if (mem_attr[mem_id].size != 0 && mem_attr[mem_id].size < MIN_SIZE_MEM_SEGMENT) {
				dprintf(CRITICAL, "[ADSP] %s memory size invalid\n", adsp_audio_mem_name[mem_id]);
				return -2;
			}
		} else {
			dprintf(CRITICAL, "[ADSP] get \"%s\" fail\n", adsp_audio_mem_name[mem_id]);
...
{
	dprintf(CRITICAL, "%s on=%d dormant=%d\n", module_prefix, on, dormant);
	if (on) {
		DRV_SetReg32(MODULE_SW_CG_3_CLR, (CG_ENABLE << CG_OFFSET));
	} else {
		DRV_SetReg32(MODULE_SW_CG_3_SET, (CG_ENABLE << CG_OFFSET));
	}
}
...
static void set_adsp_mpu_region(void)
{
	struct adsp_region_info_t *adsp_region_info = MPUINFO_REGION_BASE;

	memset(MPUINFO_REGION_BASE, 0xFF, MPUINFO_TOTAL_SIZE);
	adsp_region_info->adsp_region_start = (uint32_t)dram_phys_addr;
	adsp_region_info->adsp_region_size = (uint32_t)g_dram_size;
	dsb();
}
