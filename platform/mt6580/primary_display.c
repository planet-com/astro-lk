...
void _dump_lcm_info_by_video_printf(void)
{
	...
	if (l && p) {
		...
		if (p->type == LCM_TYPE_DSI) {
			video_printf("[LCM] LANE_NUM: %d, data_format: (%d, %d, %d, %d)\n", p->dsi.LANE_NUM, p->dsi.data_format.color_order, p->dsi.data_format.format, p->dsi.data_format.padding, p->dsi.data_format.trans_seq);
...
int primary_display_trigger(int blocking)
{
	...
	if (pgc->mode == DIRECT_LINK_MODE || pgc->mode == SINGLE_LAYER_MODE) {
		...
		if (count++ ==0)
			disp_lcm_update(pgc->plcm, 0, 0, width, height, 0);
