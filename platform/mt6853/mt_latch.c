...
int lastbus_init_v2_init_w_clr(const struct lastbus_monitor *m)
{
	...
	/* timeout_threshold: 200 ms */
	writel(0x3c170008, m->base);
	writel(0x3c17000c, m->base);
...
int lastbus_init_v2_infra_ao_init(const struct lastbus_monitor *m)
{
	...
	writel(0x1, (unsigned long)m->base + 0x004);
	writel(0x40, (unsigned long)m->base + 0x008);
	writel(0x1f86, (unsigned long)m->base + 0x00c);
	writel(0x600, (unsigned long)m->base + 0x010);
...
int lastbus_init_v2_peri_ao_init(const struct lastbus_monitor *m)
{
	...
	/* disable master/slave idle */
	writel(0x80000080, (unsigned long)m->base + 0x008);
	writel(0x3800001f, (unsigned long)m->base + 0x00c);
...
int lastbus_init_v2_fmem_ao_init(const struct lastbus_monitor *m)
{
	...
	/* disable master/slave idle */
	writel(0x00200000, (unsigned long)m->base + 0x008);

	/* timeout_threshold: 200 ms */
	writel(0x3c170008, m->base);
	writel(0x3c71000c, m->base);
