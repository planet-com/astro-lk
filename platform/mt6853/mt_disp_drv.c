...
UINT32 mt_disp_get_vram_size(void)
{
	if (mrdump_check_enable() <= MRDUMP_ALWAYS_ENABLE)
		return (DISP_GetVRamSize() + mt_disp_get_mram_size());
	else
		return DISP_GetVRamSize();
}

UINT32 mt_disp_get_mram_size(void)
{
	return DISP_GetMBRamSize();
}
...
void mt_disp_init(void *lcdbase)
{
	...
	tempfb_addr = (void *)(u32)mblock_reserve_ext(&g_boot_arg->mblock_info, (DISP_GetFBRamSize()/DISP_GetPages()),
		0x10000, 0x80000000, 0, "tempfb_addr");
	if (!tempfb_addr) {
		pal_log_err("Warning! tempfb_addr is not taken from mb\n");
		assert(0);
	}
	...
	memset((void*)lcdbase, 0x0, mt_disp_get_vram_size());
	...
	input.layer_en  = 1;
	input.fmt       = redoffset_32bit ? eBGRA8888 : eRGBA8888;
	input.addr      = fb_addr_pa + fb_offset_logo * fb_size;
	input.src_x     = 0;
	input.src_y     = 0;
	input.src_w     = CFG_DISPLAY_WIDTH;
	input.src_h     = CFG_DISPLAY_HEIGHT;
...
{
	dprintf(0, "mt_free_logo_from_mblock: %d\n", fb_offset_logo);
	if(fb_offset_logo){
		memcpy(fb_addr, (void *)((UINT32)fb_addr + 2 * fb_size), fb_size);
	...
	if (tempfb_addr) {
		mblock_create(&g_boot_arg->mblock_info,
			&g_boot_arg->orig_dram_info,
			(u64)(unsigned long)tempfb_addr, (u64)(DISP_GetFBRamSize()/DISP_GetPages()));
		tempfb_addr = NULL;
	}
}
...
void* mt_get_fb_buf(void)
{
	fb_isdirty = 1;
	if (fb_offset_logo == 0)
		return (void*)((UINT32)fb_addr + 2 * fb_size);
	else
		return (void*)((UINT32)fb_addr);
}
...
void mt_disp_update(UINT32 x, UINT32 y, UINT32 width, UINT32 height)
{
	...
	if (fb_isdirty) {
		if (fb_offset_logo == 0)
			...
			fb_offset_logo = 0;
		pa = fb_addr_pa + fb_offset_logo * fb_size;
		mt_disp_config_fb_layer(pa, TRUE);
		fb_isdirty = 0;
	}
	va = (unsigned int)mt_get_fb_addr();
	dprintf(0, "mt_disp_update: 0x%08x, 0x%08x\n", va, pa);
	...
	if (mrdump_check_enable() <= MRDUMP_ALWAYS_ENABLE &&
		current_is_abnormal_boot())
		arch_clean_cache_range((unsigned int)(fb_addr + DISP_GetVRamSize()), DISP_GetMBRamSize());
...
void mt_disp_config_fb_layer(UINT32 addr, bool en)
{
	disp_input_config input;

	dprintf(INFO, "[lk logo: %s, addr = 0x%08x\n", __FUNCTION__, addr);
...
void *video_hw_init (void)
{
	...
	if (mrdump_check_enable() <= MRDUMP_ALWAYS_ENABLE &&
		current_is_abnormal_boot()) {
		dprintf(0, "%s: mrdump boot\n", __func__);
		s_mt65xx_gd.frameAdrs  = (UINT32)fb_addr+DISP_GetVRamSize();
	}
	else {
		dprintf(0, "%s: normal boot\n", __func__);
