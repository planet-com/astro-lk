...
void DSI0_WaitNotBusyAfterTrigger(DISP_MODULE_ENUM module)
{
	int i = 0, cnt = 0;
	int delay_in_us = 50;
	unsigned int tmp = 0;
...
void DSI_exit_ULPS(DISP_MODULE_ENUM module)
{
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		if (_dsi_context[i].dsi_params.IsCphy) {
			DISP_REG_SET_FIELD(NULL, DSI_CPHY_CON0_CPHY_EN, DSI_REG_BASE[i] + DISP_REG_DSI_CPHY_CON0, 1);
...
enum DSI_STATUS DSI_BIST_Pattern_Test(DISP_MODULE_ENUM module, void* cmdq, bool enable, unsigned int color)
{
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		if (enable) {
			DSI_OUTREG32(cmdq, &DSI_REG[i]->DSI_SELF_PAT_CON1, color);
			...
			//	      SELF_PAT_MODE, 1);
			DISP_REG_SET_FIELD(cmdq, DSI_SELF_PAT_PRE_MODE, DSI_REG_BASE[i] + DISP_REG_DSI_SELF_PAT_CON0, 1);
			...
			DSI_OUTREG32(cmdq, DSI_REG_BASE[i] + DISP_REG_DSI_SELF_PAT_CON0, 0x00);
		}
...
void DSI_Config_DPHY_VDO_Timing(DISP_MODULE_ENUM module, void* cmdq, LCM_DSI_PARAMS *dsi_params)
{
	int i = 0;
...
void DSI_Config_CPHY_VDO_Timing(DISP_MODULE_ENUM module, void* cmdq, LCM_DSI_PARAMS *dsi_params)
{
	int i = 0;
	//unsigned int line_byte;
	unsigned int horizontal_sync_active_byte;
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		...
		/*data_phy_cycle = DISP_REG_GET_FIELD(DSI_PHY_TIMCON0_FLD_DA_HS_PREP, DSI_REG_BASE[i] + DISP_REG_DSI_PHY_TIMCON0) +
			DISP_REG_GET_FIELD(DSI_PHY_TIMCON0_FLD_DA_HS_ZERO, DSI_REG_BASE[i] + DISP_REG_DSI_PHY_TIMCON0) +
			DISP_REG_GET_FIELD(DSI_PHY_TIMCON1_FLD_DA_HS_EXIT, DSI_REG_BASE[i] + DISP_REG_DSI_PHY_TIMCON1) +
			DISP_REG_GET_FIELD(DSI_PHY_TIMCON0_FLD_LPX, DSI_REG_BASE[i] + DISP_REG_DSI_PHY_TIMCON0) + 5;*/
...
void DSI_CPHY_clk_setting(DISP_MODULE_ENUM module, void* cmdq, LCM_DSI_PARAMS *dsi_params)
{
	...
	unsigned int pcw = 0;
	unsigned int posdiv = 0, prediv = 0;
	...
	for (i = 0; i < 10; i++) {
		...
		if (i < 5) {
			DDPDUMP("MIPI_TX[0x%08x]: 0x%08x 0x%08x 0x%08x 0x%08x 0x%08x\n", mipitx_addr + k,
					INREG32((mipitx_addr + k)),
					INREG32((mipitx_addr + k + 0x4)),
					INREG32((mipitx_addr + k + 0x8)),
					INREG32((mipitx_addr + k + 0xC)),
					INREG32((mipitx_addr + k + 0x10)));
			DDPDUMP("MIPI_TX2[0x%08x]: 0x%08x 0x%08x 0x%08x 0x%08x 0x%08x\n", mipitx_addr2 + k,
					INREG32((mipitx_addr2 + k)),
					INREG32((mipitx_addr2 + k + 0x4)),
					INREG32((mipitx_addr2 + k + 0x8)),
					INREG32((mipitx_addr2 + k + 0xC)),
					INREG32((mipitx_addr2 + k + 0x10)));
		} else {
...
#if 0 /* LK does not rely on INTEN, use polling to handle event  */
		...
		if (0 != DSI_REG[d]->DSI_MODE_CTRL.MODE) {
			...
			if (cmd < 0xB0) {
				if (count > 1) {
					...
					mask_para = (0xFFU<<((goto_addr&0x3)*8));
					...
					for (i=0; i<count; i++) {
						...
						mask_para = (0xFFU<<((goto_addr&0x3)*8));
						set_para = ((unsigned)para_list[i]<<((goto_addr&0x3)*8));
						DSI_MASKREG32(cmdq, goto_addr&(~0x3), mask_para, set_para);
					}
				} else {
					vm_cmdq.LONG_PKT = 0;
			...
			if (0 != DSI_REG[d]->DSI_MODE_CTRL.MODE) {
				...
				if (count > 1) {
					...
					for (i=0; i<count; i++) {
						...
						mask_para = (0xFFu<<((goto_addr&0x3u)*8));
						set_para = ((unsigned)para_list[i]<<((goto_addr&0x3u)*8));
						DSI_MASKREG32(cmdq, goto_addr&(~((unsigned long)0x3u)), mask_para, set_para);
					}
...
{
#if 0
	OUTREG32(MMSYS_CONFIG_BASE+0x180, value);
