...
void set_write_protect(void)
{
	...
	dev = (part_dev_t *)mt_part_get_device();
	...
	if (system_wp) {
		...
		if (ab_suffix[1] == 'a') {
			s = snprintf(wp_start, WRITE_PROTECT_PARTITION_NAME_SZ, "md1img_a");
			e = snprintf(wp_end, WRITE_PROTECT_PARTITION_NAME_SZ, "logo");
		} else {
			s = snprintf(wp_start, WRITE_PROTECT_PARTITION_NAME_SZ, "sec1");
			if (PART_OK == partition_exists("system_b"))
				e = snprintf(wp_end, WRITE_PROTECT_PARTITION_NAME_SZ, "system_b");
			else
				e = snprintf(wp_end, WRITE_PROTECT_PARTITION_NAME_SZ, "tee_b");
		}
	...
	if (protect2_wp)
		s = snprintf(wp_start, WRITE_PROTECT_PARTITION_NAME_SZ, "protect2");
	else
		s = snprintf(wp_start, WRITE_PROTECT_PARTITION_NAME_SZ, "seccfg");

	e = snprintf(wp_end, WRITE_PROTECT_PARTITION_NAME_SZ, "seccfg");
