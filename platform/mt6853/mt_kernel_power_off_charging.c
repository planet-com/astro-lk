...
BOOL is_force_boot(void)
{
	if (rtc_boot_check(true)) {
		...
		return TRUE;
	} else if (g_boot_arg->boot_reason == BR_POWER_KEY ||
		   g_boot_arg->boot_reason == BR_LONG_POWKEY ||
		   g_boot_arg->boot_reason == BR_POWER_EXC ||
		   g_boot_arg->boot_reason == BR_POWER_LOSS ||
		   g_boot_arg->boot_reason == BR_REBOOT_EXCEPTION ||
