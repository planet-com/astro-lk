...
void rdma_cal_golden_setting(unsigned int idx, unsigned int width,
	...
	unsigned int *gs, unsigned int is_vdo)
{
	/* fixed variable */
	...
	if(frame_rate)
		if_fps = frame_rate;
	DDPMSG("RDMA1 golden setting if_fps= %d\n", if_fps);
#if 0
	...
	void * handle)
{
	 ...
	 */
	rdma_cal_golden_setting(idx, width, height, bpp, frame_rate, gs, (dsi_mode > 0));
               ...
               void * handle)  // ourput setting
{
	...
	rdma_set_ultra(idx, width, height, bpp, fps, dsi_mode, handle);
#if 0
