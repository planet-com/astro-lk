...
#define MT_MD_WDT1_IRQ_ID   (78+GIC_PRIVATE_SIGNALS)


//------- register part ---------------------------------------
#define MD1_BUS_PROTECT_SET   (0x10001B84)
#define MD1_BUS_PROTECT_CLR   (0x10001B88)
#define MD1_BUS_PROTECT_STA   (0x10001B90)
#define MD1_BUS_PROTECT1_SET  (0x100012A0)
#define MD1_BUS_PROTECT1_CLR  (0x100012A4)
#define MD1_BUS_PROTECT1_STA  (0x10001228)
#define MD1_BUS_PROTECT2_SET  (0x10001BB8)
#define MD1_BUS_PROTECT2_CLR  (0x10001BBC)
#define MD1_BUS_PROTECT2_STA  (0x10001BC4)
...
#define UART1_TX_GPIO_ID (GPIO95 | GPIO_MAGIC)
#define UART1_RX_GPIO_ID (GPIO94 | GPIO_MAGIC)
#define UART2_TX_GPIO_ID (GPIO149 | GPIO_MAGIC)
#define UART2_RX_GPIO_ID (GPIO147 | GPIO_MAGIC)
#define UART3_TX_GPIO_ID (GPIO37 | GPIO_MAGIC)
#define UART3_RX_GPIO_ID (GPIO36 | GPIO_MAGIC)
...
void bus_protection_en(int md_id)
{
	if (md_id == MD_SYS1) {
		...
		DRV_WriteReg32(MD1_BUS_PROTECT2_SET, PROTECTION2_BITMASK);
		...
		dprintf(CRITICAL, "wait protection2 ....\n");
		while ((DRV_Reg32(MD1_BUS_PROTECT2_STA)&PROTECTION2_BITMASK) != PROTECTION2_BITMASK) {
...
void bus_protection_diable(int md_id)
{
	if (md_id == MD_SYS1) {
		...
		DRV_WriteReg32(MD1_BUS_PROTECT2_CLR, PROTECTION2_BITMASK);
		/* poll protection ready */
		...
		dprintf(CRITICAL, "wait protection2 disable....\n");
		while ((DRV_Reg32(MD1_BUS_PROTECT2_STA)&PROTECTION2_BITMASK) != 0x00000000) {
			dprintf(CRITICAL, "0x%x\n", DRV_Reg32(MD1_BUS_PROTECT2_STA));
		}
...
static void config_md_boot_env(int md_id, int boot_mode)
{
	...
	reg_value = spm_read(SPM_POWER_ON_VAL1);
	reg_value |= (0x1 << 21);
	spm_write(SPM_POWER_ON_VAL1, reg_value);
...
#ifndef NO_UNGATE_MD
			/* default trigger md run*/
			lk_secure_ret = mt_secure_call_ret4(MTK_SIP_BL_CCCI_CONTROL_AARCH32,
				MD_POWER_CONFIG, MD_LK_BOOT_UP_FOR_UNGATE, 1, 0, 0, 0, 0);
			dprintf(CRITICAL, "lk_secure_ret=%d\n", lk_secure_ret);
#else
...
#define BRINGUP_BYPASS_BROM  1
int bypass_md_boot_rom(void)
{
	unsigned int res_bypass = 0;
	res_bypass = mt_secure_call_ret4(MTK_SIP_BL_CCCI_CONTROL_AARCH32,
		MD_POWER_CONFIG, MD_LK_BYPASS_BROM, BRINGUP_BYPASS_BROM, 0, 0, 0, 0);
	if (res_bypass)
		dprintf(CRITICAL, "do bypass md brom\n");
	else
		dprintf(CRITICAL, "no bypass md brom,normal flow\n");
