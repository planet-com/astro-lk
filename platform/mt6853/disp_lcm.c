...
int disp_lcm_fill_lcm_dts_phandle(void* fdt, int idx)
{
	...
	DISPMSG("dsi panel target phandle:0x%x\n", phandle);
	...
	if (val) {
		...
		DISPMSG("default dsi remote-endport is 0x%x\n before lk set \n", data);
	} else {
	...
	if (val) {
		memcpy(&data, val, sizeof(data));
		data = fdt32_to_cpu(data);
		DISPMSG("set dsi remote-endport as 0x%x done \n", data);
	} else {
		DISPERR("dsi remote-endport is empty after set \n");
	}
	if (dts_is_cphy) {
...
void _lcm_fake_resolution_display(disp_lcm_handle *plcm)
{
	void *lk_drv_fdt = get_lk_overlayed_dtb();
	unsigned int fake_high = 0;
	unsigned int fake_width = 0;
	char buf[100];
	int ret;

	if (lk_drv_fdt) {
		ret = snprintf(buf, sizeof(buf) - 1, "/dsi/panel%d", dts_panel_index);
		if (ret < 0 || ret >= sizeof(buf) - 1) {
			DISPERR("%s,snprint dst_panel failed\n", __func__);
			return;
		}
		fake_high = disp_lcm_get_dts_u32(lk_drv_fdt, buf, "fake_heigh");
		fake_width = disp_lcm_get_dts_u32(lk_drv_fdt, buf, "fake_width");
		if (fake_width > 0 && fake_width < plcm->params->width) {
			plcm->params->virtual_width = plcm->params->width;
			plcm->params->width = fake_width;
		}
		if (fake_high > 0 && fake_high < plcm->params->height) {
			plcm->params->virtual_height = plcm->params->height;
			plcm->params->height = fake_high;
		}
	}
	return;
}
...
disp_lcm_handle* disp_lcm_probe(char* plcm_name, LCM_INTERFACE_ID lcm_id)
{
	...
	_lcm_fake_resolution_display(plcm);
	_dump_lcm_info(plcm);
	return plcm;
