...
#define CCCI_SMEM_SIZE_SECURITY (1*1024*1024) /* security smem total size */
...
static const unsigned int smem_align_ref[SMEM_USER_LAST] = {
	...
	0, /* SMEM_USER_LOW_POWER */
	1024*1024, /*SMEM_USER_AP_MD1_SECURITY_SMEM,*/
};
...
static int cal_nc_share_mem_size(void)
{
	...
	/*CCCI_SMEM_AP_MD_SECURITY*/
	size = CCCI_SMEM_SIZE_SECURITY;
	align_sz = get_align_sz_by_user(nc_size, SMEM_USER_SECURITY_SMEM);
	ALWAYS_LOG("[NC] align size for SECURITY: 0x%x\n", align_sz);
	nc_size += align_sz;
	offset = nc_size;
	nc_size += size;
	ALWAYS_LOG("[NC] block: AP_MD1_SECURITY: 0x%x(0x%x)<0x%x>\n", offset, size, nc_size);
	update_nc_smem_info(SMEM_USER_SECURITY_SMEM, offset, offset, size);
	...
	if (cacheable_buf_sz == 0) {
		...
		for (i = 0; i < sizeof(csmem_layout)/sizeof(csmem_item_t); i++) {
			...
			if (csmem_layout[i].item_cnt == SMEM_USER_CCB_START)
				ccb_data_buf = (unsigned char *)((unsigned long)csmem_layout[i].csmem_buffer_addr);
		}
...
#define MPU_REGION_ID_MD1_SECURE_SHARE  8
...
static const mpu_att_t mpu_att_default[MPU_REGION_ID_TOTAL_NUM][MPU_MDOMAIN_ID_TOTAL_NUM] = {
	...
	/* 7*/{}, /*Trusted UI*/
	/* 8*/{ NO_PROTECTION,   NO_PROTECTION,  [2 ... 15] = FORBIDDEN}, /*Security CCCI share memory*/
	/* 9*/{}, /*AMMS*/
...
#ifdef DUMMY_AP_MODE
	DRV_WriteReg32(0x10209100, (md_ld_info->base_addr >> 24));
#endif
...
int mtk_ccci_get_security_smem_info(unsigned long long *ap_addr, unsigned int *md_addr, unsigned int *size)
{
	unsigned int ap_offset;
	unsigned int md_offset;
	unsigned int smem_size;
	...
	if (get_nc_smem_info(SMEM_USER_SECURITY_SMEM, &ap_offset, &md_offset, &smem_size) < 0)
		return -1;
...
int plat_apply_platform_setting(int load_md_flag)
{
	...
	if (get_nc_smem_info(SMEM_USER_SECURITY_SMEM, &ap_offset, &md_offset, &smem_size) < 0)
		return -1;
	mpu_tbl[s_g_curr_mpu_num].region = MPU_REGION_ID_MD1_SECURE_SHARE;
	get_mpu_region_default_access_att(mpu_tbl[s_g_curr_mpu_num].permission, MPU_REGION_ID_MD1_SECURE_SHARE, 0);
	mpu_tbl[s_g_curr_mpu_num].start = (unsigned int)smem_info.base_addr + ap_offset;
	mpu_tbl[s_g_curr_mpu_num].end = (unsigned int)smem_info.base_addr + ap_offset
									+ CCCI_SMEM_SIZE_SECURITY;
	mpu_tbl[s_g_curr_mpu_num].end = ((mpu_tbl[s_g_curr_mpu_num].end + 0xFFFF)&(~0xFFFF)) - 1;
	s_g_curr_mpu_num++;
