...
static int verify_load_scp_image(char *part_name, void *addr)
{
	...
	fw_size = img_size - SCP_LOADER_SIZE;
	...
	if (scp_dram_img_size > 0) {
		dprintf(INFO, "[SCP] load_scp_image %s %d bytes (max size= 0x%x).\n",
		              DRM_NAME_SCP_A, scp_dram_img_size, SCP_DRAM_IMG_SIZE);

		/* assert if size of feature placed in dram plus size of overlay exceeds
		 * size, SCP_DRAM_IMG_SIZE. */
		if (scp_dram_img_size > SCP_DRAM_IMG_SIZE)
			ASSERT(0);

		// step 3.1 backup scp_dram_img code
		scp_dram_backup_ptr = scp_dram_img_ptr + roundup(scp_dram_img_size, 1024);
		/* mt6853 don't have core1 dupicate dram region, we can remove *2 */
	...
	if (img_size > scp_sram_size) {
		scp_region_info->ap_firmware_size = scp_sram_size - SCP_LOADER_SIZE;
