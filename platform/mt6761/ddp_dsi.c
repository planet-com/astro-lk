...
#define NS_TO_CYCLE(n, c)   ((n) / (c))
	...
	hs_trail_n= (dsi_params->HS_TRAIL == 0) ?
		(NS_TO_CYCLE(((hs_trail_m * 0x4 * ui) + 0x50) * dsi_params->PLL_CLOCK * 2, 0x1F40) + 0x1) : dsi_params->HS_TRAIL;
	// +3 is recommended from designer becauase of HW latency
	timcon0.HS_TRAIL = (hs_trail_m > hs_trail_n) ? hs_trail_m : hs_trail_n;

	timcon0.HS_PRPR = (dsi_params->HS_PRPR == 0) ? (NS_TO_CYCLE((0x40 + 0x5 * ui), cycle_time) + 0x1) : dsi_params->HS_PRPR;
	// HS_PRPR can't be 1.
	...
	timcon0.LPX = (dsi_params->LPX == 0) ? (NS_TO_CYCLE(dsi_params->PLL_CLOCK * 2 * 0x4B, 0x1F40) + 0x1) : dsi_params->LPX;
	if (timcon0.LPX < 1)
		timcon0.LPX = 1;
	...
	timcon2.CLK_TRAIL = ((dsi_params->CLK_TRAIL == 0) ? NS_TO_CYCLE(0x64 * dsi_params->PLL_CLOCK * 2, 0x1F40) : dsi_params->CLK_TRAIL) + 0x01;
	...
	timcon3.CLK_HS_PRPR = (dsi_params->CLK_HS_PRPR == 0) ? NS_TO_CYCLE(0x50 * dsi_params->PLL_CLOCK * 2, 0x1F40) : dsi_params->CLK_HS_PRPR;
